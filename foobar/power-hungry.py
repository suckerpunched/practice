...
def solution(xs):
    z = None
    if len(xs) == 2 or (len(xs) == 2 and 0 in xs): return str(max(xs))
    if 0 in xs: z = str(0)
    
    p = filter(lambda x: x > 0, xs)
    n = filter(lambda x: x < 0, xs)

    if n: 
        if len(n) % 2 == 0: return str(reduce(mul, n+p, 1))
        r = max(n)
        n.remove(r)
        
        if n: return str(reduce(mul, n+p, 1))
    
    if p: return str(reduce(mul, p, 1))
    return z or str(r) 
